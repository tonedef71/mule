...
public class EventCorrelator implements Startable, Stoppable, Disposable
{
    ...
    protected void handleGroupExpiry(EventGroup group) throws MessagingException
    {
        ...
        if (isFailOnTimeout())
        {
            MuleEvent messageCollection = group.getMessageCollectionEvent();
            muleContext.fireNotification(new RoutingNotification(messageCollection.getMessage(), null,
            ...
            throw new CorrelationTimeoutException(CoreMessages.correlationTimedOut(group.getGroupId()), messageCollection);
        }
        else
        {
            if (logger.isDebugEnabled())
                    ...
                    if (!correlatorStore.contains((Serializable) group.getGroupId(), getExpiredAndDispatchedPartitionKey()))
                    {
                        ...
                        else
                        {
                            throw new MessagingException(CoreMessages.createStaticMessage(
                                    MessageFormat.format("Group {0} timed out, but no timeout message processor was " +
                                                         "configured.", group.getGroupId())), newEvent);
                        }
